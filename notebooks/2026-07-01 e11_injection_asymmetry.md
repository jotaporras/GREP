...
- **Task:** SPINE/PRISM navigation planning; compact prom*pt lists node names + robot
...
`rpearl_gt_llm`), structural attention masks (`graph_mask_llm`,@ `learnable_graph_mask` with $M = \alpha A + (1-\alpha)\mathrm{sim}(\Psi\Psi^\top)$),
...
$$
\underbrace{B^{\text{train}}*{t\cdot}, B^{\text{train}}*{\cdot t} = M_{\pi(t)\cdot}}*{t \in \text{answer mentions}}
\qquad\text{vs.}\qquad
B^{\text{gen}}*{t\cdot} = B^{\text{gen}}_{\cdot t} = 0 .
$$
...
> triples the probability on the correct next node ($e^{-0.92}\approx0.40$ vs
> $e^{-1.96}\approx0.14$); prompt-only retains ~1 pt. `e10_integ_rpe` (pretrained
...
> A/B (e.g. confirm the prompt_only collator path was active — `[data] injection_scope=prompt_only` printed in both slurm logs); with-edges parity rerun
