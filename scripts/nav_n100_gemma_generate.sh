#!/usr/bin/env bash
# Generate navigation-only (~100-node) training data with a LOCAL Gemma 4
# model via HF Transformers — no OpenAI calls in either phase.
#
#   * Graph size : 7 communities x 11 nodes = 77 regions + ~30% objects ~= 100 nodes.
#                  (total nodes = n_regions x (1 + object_rate); objects are
#                   Poisson(object_rate) per region — see generate_eval_graphs.py)
...
#
# Usage: bash scripts/nav_n100_gemma_generate.sh [GPU_ID] [MODEL] [N_GRAPHS]
