...
def test_extract_graph_decode_consistent_query_and_key_maps():
    ...
    for nid, spans in imap.items():
        full_spans = dict((s, e) for s, e in full[nid])
        for s, e in spans:
            if s >= a:
                assert e - s == 1
                assert any(fe == e for fs, fe in full[nid])   # final token of a full span
            else:
                assert (s, full_spans[s]) == (s, e)           # prompt spans untouched
    # Coverage: every node with a full-map answer span still has exactly one
    # query position per answer mention.
    for nid in full:
        n_answer_full = sum(1 for s, _ in full[nid] if s >= a)
        n_answer_q = sum(1 for s, _ in imap[nid] if s >= a)
        assert n_answer_q == n_answer_full
