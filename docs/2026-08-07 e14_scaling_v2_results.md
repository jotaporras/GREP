...
`Merge lora module to 4-bit linear may get different generations due to rounding errors` (`peft/tuners/lora/bnb.py:397`) during every reload —
...
- **Rerun with the fix — gate passed, probes valid.** 7439396 (n30 GT on
test graphs, fixed 4-bit path) scored **0.729** — between in-training
0.700 and bf16 0.757, ~13× the broken path — so the fix is confirmed and
the probe2 numbers below are trustworthy.
...
## 6. SPINE tools-enabled eval (n30 arms, added 2026-08-10)

Every accuracy above was measured with SPINE tool-calling **disabled**
(`PRISM_DISABLE_SPINE_TOOLS=1`): the model writes a route directly and nothing
executes. This section is the tools-**enabled** counterpart, run via
`scripts/e14_transferability_spine.sbatch` — the model may call the SPINE API
actions (`goto`, `map_region`, …) against a live `GraphSim` while it plans.
Same checkpoints, same 7 held-out graphs, same scoring; only the prompt policy
...
edge list it was trained with — a silent train/eval mismatch. `TEXT_EDGE_LIST`
is now an env override, and the value was verified per arm from the output
JSONs (`present`/`llm` on edges, `none`/`graph-augmented` on GT) rather than
assumed.
